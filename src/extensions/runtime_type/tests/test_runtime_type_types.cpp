...
static LookupResult lookup_found(Type ty, std::u8string_view name, Type expected)
{
...
TEST_CASE("stdcolt/extensions/runtime_type: lookup_fast vs lookup + prepared member")
{
  ...
  REQUIRE(r1.result == LookupResult::LOOKUP_FOUND);

  auto r2 = rt_type_lookup_fast(ty, {u8"x", 1}, expected);
  REQUIRE(r2.result == LookupResult::LOOKUP_FOUND);
          ...
          "member on failure")
{
  ...
  REQUIRE(off_a.result == LookupResult::LOOKUP_FOUND);
  REQUIRE(off_b.result == LookupResult::LOOKUP_FOUND);
          ...
          "LOOKUP_EXPECTED_NAMED")
{
  ...
  for (Type t : {t_i32, t_ptr, t_arr, t_fn})
  {
    ...
    CHECK(r.result == LookupResult::ResultKind::LOOKUP_EXPECTED_NAMED);

    auto rf = rt_type_lookup_fast(t, {u8"x", 1}, expected);
    CHECK(rf.result == LookupResult::ResultKind::LOOKUP_EXPECTED_NAMED);
  }
          ...
          "rollback recursion")
{
          ...
          "copy fails")
  {
    ...
    REQUIRE(off_a.result == LookupResult::ResultKind::LOOKUP_FOUND);
    REQUIRE(off_i.result == LookupResult::ResultKind::LOOKUP_FOUND);
    ...
    REQUIRE(off_b.result == LookupResult::ResultKind::LOOKUP_FOUND);
    REQUIRE(off_c.result == LookupResult::ResultKind::LOOKUP_FOUND);
